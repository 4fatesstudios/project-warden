...
namespace FourFatesStudios.ProjectWarden.RelationshipSystem
{
    ...
    public class RelationshipTracker : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // Using Start() instead of Awake() to ensure RelationshipManager.Instance is initialized
            Debug.Log($"[RelationshipTracker] Start called for character: '{characterName}'");
            
            if (string.IsNullOrEmpty(characterName))
            {
                Debug.LogWarning($"[RelationshipTracker] Character name is null or empty on GameObject: {gameObject.name}");
                return;
            }
            
            if (RelationshipManager.Instance == null)
            {
                Debug.LogWarning($"[RelationshipTracker] RelationshipManager.Instance is null when trying to register '{characterName}'");
                return;
            }
            
            RelationshipManager.Instance.RegisterCharacter(characterName, this);
            Debug.Log($"[RelationshipTracker] Successfully registered character '{characterName}' with RelationshipManager");
        }
