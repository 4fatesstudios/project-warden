...
namespace FourFatesStudios.ProjectWarden.RelationshipSystem
{
    public class RelationshipManager : MonoBehaviour
    {
        ...
        {
            Debug.Log($"[RelationshipManager] Attempting to register character: '{characterName}'");
            
            if (string.IsNullOrEmpty(characterName) || tracker == null)
            {
                Debug.LogWarning($"[RelationshipManager] Invalid character name ('{characterName}') or tracker (null: {tracker == null}) provided for registration.");
                return;
            }
            
            characterRelationships[characterName] = tracker;
            Debug.Log($"[RelationshipManager] Successfully registered character '{characterName}'. Total registered characters: {characterRelationships.Count}");
            
            // Log all registered characters for debugging
            Debug.Log($"[RelationshipManager] Currently registered characters: {string.Join(", ", characterRelationships.Keys)}");
        }
        ...
        /// <summary>
        /// Debug method to list all registered characters
        /// </summary>
        [ContextMenu("List All Registered Characters")]
        public void ListAllRegisteredCharacters()
        {
            Debug.Log($"[RelationshipManager] Total registered characters: {characterRelationships.Count}");
            foreach (var kvp in characterRelationships)
            {
                Debug.Log($"[RelationshipManager] Character: '{kvp.Key}' - Status: {kvp.Value.relationshipStatus} - Points: {kvp.Value.relationshipPoints}");
            }
        }
    }
}
